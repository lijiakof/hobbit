...
$.extend(Date.prototype, {
    ...
    , add: function (v, part) { if (part == 1 || part == 2) { var r = new Date(this); if (part == 1) r.setYear(r.getFullYear() + v); else r.setMonth(r.getMonth() + v); return r; } else return new Date(this.getTime() + [1, 0, 0, Date.MSINDAY, Date.MSINHOUR, Date.MSINMINUTE, Date.MSINSECOND, 1][part || 7] * v); }
    //返回日期的部分，默认年月日，可以通过part参数指定精度
    //part:1-年，2-月，3-日(默认)，4-时，5-分，6-秒，7-毫
    , date: function (part) { var t = this.toArray(); for (var i = part || 3; i < 7; i++) t[i] = i == 2 ? 1 : 0; return new Date(t[0], t[1], t[2], t[3], t[4], t[5], t[6]); }
    //将时间转换为数组
    ...
    //part:1-年，2-月，3-日，4-时，5-分，6-秒，7-毫(默认)
    , equal: function (date, part) { if (typeof this != typeof date) return false; var t = this.toArray(); date = date.toArray(); for (var i = 0; i < (part || 7) ; i++) if (t[i] != date[i]) return false; return true; }
    //计算两个日期的间隔，可以通过part参数指定哪部分
    //part:1-年，2-月，3-日，4-时，5-分，6-秒，7-毫(默认)
    , diff: function (date, part) { if (part == 1 || part == 2) { var r = this.getFullYear() - date.getFullYear(); return part == 2 ? (r * 12 + this.getMonth() - date.getMonth()) : r; } part = [1, 0, 0, Date.MSINDAY, Date.MSINHOUR, Date.MSINMINUTE, Date.MSINSECOND, 1][part || 7]; return this.getTime() / part - date.getTime() / part; }
});
