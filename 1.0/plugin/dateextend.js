...
$.extend(Date.prototype, {
    ...
    //mode:1-年，2-月，3-日，4-时，5-分，6-秒，7-毫
    add: function (v, mode) { if (mode == 1) { mode = new Date(this); return mode.setYear(mode.getFullYear() + v); } else if (mode == 2) { mode = new Date(this); return mode.setMonth(mode.getMonth() + v); } else return new Date(this.getTime() + [1, 0, 0, Date.MSINDAY, Date.MSINHOUR, Date.MSINMINUTE, Date.MSINSECOND, 1][mode || 7] * v); },
...
$.extend(Date, {
    //转换对象、字符串或数字为日起对象
    parseDate: function (v, d) { if (typeof v == "object" || typeof v == "number") v = new Date(v); else if (typeof v == "string") v = new Date(v.trim().replace(/\-/g, "/")); return v == "Invalid Date" ? d : v; },
    MSINSECOND: 1000, MSINMINUTE: 60000, MSINHOUR: 3600000, MSINDAY: 86400000
});